...
export class TodoListVM {
  ...
  private seed() {
    const samples = [
      ["Ship mobx-solid SSR playground", false],
      ["Wire enableObservableTracking on server + client", true],
      ["renderToString MobX state", false],
      ["Hydrate without losing reactivity", true],
      ["Add filter & pagination", true],
      ["Simulate async sync", false],
      ["Theme via local MobX store (SSR-safe)", false],
    ] as const;
